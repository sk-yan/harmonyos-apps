...
| 应用 | 用途 | 当前版本 | 运行状态 | 入口 |
| --- | --- | --- | --- | --- |
| 一笔记账 | 本地记账、微信/支付宝账单导入、截图辅助录入 | `1.1.0-preview.2` | 开发预览；公共包未签名；真机基础检查 | [源码与功能](apps/yibi-ledger/) · [版本发布](https://github.com/sk-yan/harmonyos-apps/releases/tag/yibi-ledger-v1.1.0-preview.2) |

**当前为开发预览版，公开下载的 HAP 未签名。** 开发测试使用 HarmonyOS 6.1.0（API 23）、Mate X5 折叠屏模拟器；各版本实际完成的检查见[验证说明](docs/VALIDATION.md)。Release 附件用于开发和模拟器测试，已在 Mate X6（API 24）完成本地调试签名安装、外屏账单导入/去重/撤销，以及系统图片 OCR、确认保存和删除的重点流程验证。个人调试签名包不公开分发。GitHub 发布不等于手机可直接安装或应用市场上架。
...
原生图片 OCR 使用系统能力，官方不支持在模拟器验证，Mate X6 已通过单张合成 PNG 的识别、核对、保存和删除验证；模拟器可使用粘贴文字入口。
...
前两张截图来自 Mate X6 外屏实测，其余为模拟器或浏览器验证图；展示的都是合成测试账单；应用初次启动为空账本。数据保存在当前应用沙箱中，浏览器预览另存一份独立账本。当前没有账号、云同步或数据导出，卸载或清除数据会删除本机账目。1.1 可读取旧版账本；使用导入功能后应避免降级，旧版保存时会丢失导入身份信息。
