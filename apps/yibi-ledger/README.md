...
[版本发布](https://github.com/sk-yan/harmonyos-apps/releases/tag/yibi-ledger-v1.1.0-preview.2) · [构建与安装](docs/INSTALL.md) · [改进记录](../../docs/IMPROVEMENTS-1.1.md) · [返回应用目录](../../README.md)

**当前为 `1.1.0-preview.2` 开发预览版，公开下载的 HAP 未签名。** 原生版本为 `1.1.0`，版本码为 `1001001`。模拟器验收使用 HarmonyOS 6.1.0（API 23）、Mate X5 折叠屏模板。已在 Mate X6（API 24）完成本地调试签名安装、外屏账单导入/去重/撤销，以及系统图片 OCR、确认保存和删除验证。个人调试签名包不公开分发。
...
2. 点击「导入账单」，选择文件；也可粘贴含表头的 CSV 或表格文字。支持 UTF-8 / GBK CSV，以及普通未加密 XLSX（读取第一个可见工作表）。每个文件最多 **8 MiB、5,000 笔明细**；不支持旧 XLS、加密文件或含公式的工作表。
...
原生版使用系统 Core Vision OCR；该能力不支持模拟器，**已在 Mate X6（API 24）通过合成单笔图片的真实 OCR 与确认保存验证**，模拟器可粘贴文字走完整核对流程。浏览器版在本机运行中英文 OCR，脚本与模型从当前预览站点加载，图片不会发送给外部识别服务。浏览器识别结果同样必须人工核对。
...
<p>
  <img src="docs/screenshots/phone-empty.png" width="235" alt="外屏空账本" />
...
前两张截图来自 Mate X6 外屏实测，其余图标明模拟器或浏览器来源，样例均为合成测试账单；测试后已清空。应用默认不附带示例账目。
...
1.1 已通过 58 项共享核心/存储/XLSX 测试、8 项构建辅助脚本测试、类型检查和预览构建。浏览器原有 9 组与新增 12 组串联通过，生产静态页面也实际通过 CSV 和中文 OCR 检查。原生基础回归 8 组通过；导入相关 8 项按“文件平台 6 项 + 最终键盘与文字草稿 2 项”分段完成，不表示同一轮完整脚本一次通过。详情见[本次验证记录](docs/verification-1.1-preview.2.json)和[改进记录](../../docs/IMPROVEMENTS-1.1.md)。
